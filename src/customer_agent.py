...
2. 格式完整：`[成交:產品名稱:金額]`（金額用純阿拉伯數字，不要逗號、不要「萬」字；
   這個金額就是你上面「金額決策」裡決定要投的數字）
3. 整體回應控制在 100 字內避免被截斷

範例（正確）：
```
[成交:穩健月配息基金:500000]
好，這個保本我比較安心，我先投 50 萬試試看。
```

未決定成交時正常對話，不要寫 `[成交:...]`。
**只有理專推對商品方向、而且真的打動你時才成交。方向錯、或你不夠安心就拒絕或只投小錢。**"""
...
- ✅ **算成交**（必須是當下就拍板的明確承諾）：
  - 「好，就這個方案，我投 XX 萬」「我決定買了」「幫我辦」「我們簽約」「錢我現在就轉給你」
  - 或客戶回應裡有 `[成交:產品:金額]` 標記
- ❌ **算未成交**（以下全部都是「還沒成交」，不要被「語氣正面」騙了）：
  - 「我再想想」「下次再聊」「回去考慮」「我跟家人討論一下」
  - **附條件 / 未來式的承諾**：「你下次方案做好我『就會』撥錢」「五天後你來，東西好我『再』決定」
    「你先做功課，我看了滿意『再說』」「等你給我清單我『才』考慮」——這些都是**還沒成交**，
    因為交易還卡在一個尚未發生的條件上。
  - 客戶只是「願意給理專一次機會 / 再見一次面 / 給時間」≠ 已成交。
  - 明確拒絕、或只是禮貌客套、稱讚理專。
- 🔑 判斷準則：問自己「**這通電話掛掉的當下，錢有沒有真的要動？**」。
  如果還要等下一次見面、下一份方案、客戶再評估一次——就是**未成交**。
...
### 💵 客戶投入金額：XXX
如果成交，寫出**客戶明確承諾「要投入這筆交易」的金額**（純阿拉伯數字，台幣，不要逗號不要「萬」字）。
- 例如客戶說「好，我投 688 萬」就寫 6880000；說「先放 300 萬試試」就寫 3000000。
- ⚠️ **絕對不要**把客戶提到的「總資產 / 身價 / 手上有多少閒錢」當成投入金額。
  例如客戶說「我手上有四五千萬」——那是他的**身家**，不是他答應投入的錢。
  除非他明確說「這四五千萬我都交給你」，否則**不可以**寫 45000000。
- 金額必須來自客戶**親口承諾要投入**的那句話；若找不到這樣的句子，就代表這根本不是成交，回頭把判定改成「未成交」。
