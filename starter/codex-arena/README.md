...
**症狀**：在 Windows（PowerShell）上，第一次跟客戶對話時送出的中文變成 `?`。

**原因**：PowerShell 預設編碼不是 UTF-8，中文在送出前就被轉成 `?` 了。

**解法**：把下面這段**直接貼給 Codex**：

> 送 MCP 請求時，請先把整個 request body 明確編碼成 UTF-8 bytes 再送（例如 PowerShell 用
> `[System.Text.Encoding]::UTF8.GetBytes($json)` 取得 bytes，再用 `Invoke-RestMethod -Body $bytes
> -ContentType "application/json; charset=utf-8"` 送出），不要把中文字串直接交給 curl。
